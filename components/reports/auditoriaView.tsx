...
import { 
  ...
  ChevronDown, ChevronUp, History, Gift
} from 'lucide-react'
          ...
          <div onClick={() => setExpandedId(expandedId === audit.id ? null : audit.id)} className="p-6 flex items-center justify-between cursor-pointer hover:bg-slate-800/30 transition-colors">
            <div className="flex items-center gap-6">
              <div className="text-left">
                <span className="text-[12px] font-black text-indigo-400 uppercase italic">Jornada Auditada</span>
              ...
              <div className="hidden md:block text-left">
                <span className="text-[12px] font-black text-slate-200 uppercase">Balance de Stock</span>
                <p className={`text-xs font-black uppercase italic ${Math.abs(audit.totalDiferencia) < 1 ? 'text-emerald-500' : 'text-amber-500'}`}>
                  {Math.abs(audit.totalDiferencia) < 1 ? 'Sin desvíos' : `Diferencia: ${audit.totalDiferencia.toFixed(1)} u.`}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
               <span className="text-[12px] text-slate-200 font-bold uppercase hidden sm:block italic">Cerrado el {format(new Date(audit.fechaCarga), "dd/MM HH:mm")}</span>
            ...
            <div className="px-6 pb-6 animate-in zoom-in-95 duration-200">
              <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/50">
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-[12px] font-black text-slate-200 uppercase border-b border-slate-800">
                      <th className="px-4 py-3">Insumo</th>
                      <th className="px-4 py-3 text-center">Ventas</th>
                      <th className="px-4 py-3 text-center">Regalos</th>
                      <th className="px-4 py-3 text-center text-indigo-400">Físico</th>
                  ...
                  <tbody>
                    {audit.productos?.map((p: any, i: number) => (
                      <tr key={i} className="border-b border-slate-800/30 last:border-0 hover:bg-white/5 transition-colors">
                        <td className="px-4 py-3 text-[15px] font-black text-slate-300 uppercase italic">{p.nombre}</td>
                        <td className="px-4 py-3 text-[17px] text-center font-bold text-rose-500">-{p.vendidos?.toFixed(1)}</td>
                        <td className="px-4 py-3 text-[17px] text-center font-bold text-purple-400">-{p.regalados?.toFixed(1) || '0.0'}</td>
                        <td className="px-4 py-3 text-[17px] text-center font-black text-indigo-400">{p.fisico?.toFixed(1)}</td>
                        <td className={`px-4 py-3 text-[17px] text-right font-black ${Math.abs(p.diferencia) > 0.1 ? 'text-amber-500' : 'text-emerald-500'}`}>
...
export function AuditoriaView() {
  ...
  // ✅ OPTIMIZACIÓN: Pre-calculamos los totales para que la tabla sea fluida
  const totalsByBottle = useMemo(() => {
    const map: Record<string, { vendidos: number, regalados: number }> = {}
    
    movements.forEach(m => {
      if (!map[m.botellaId]) map[m.botellaId] = { vendidos: 0, regalados: 0 }
      
      const tipo = (m.tipo as string).toLowerCase()
      const cantidad = Math.abs(Number(m.cantidad) || 0)
      
      if (tipo === 'venta') {
      map[m.botellaId].vendidos += cantidad

          } else if (tipo === 'egreso') {
            map[m.botellaId].vendidos += cantidad

          } else if (tipo === 'cortesia' || tipo === 'regalo') {
            map[m.botellaId].regalados += cantidad
          }
    })
    return map
  }, [movements])
  ...
  const handleFinalizar = async () => {
    const confirmacion = confirm(`¿Finalizar jornada del ${fechaJornada}?`);
    if (!confirmacion) return;
    setIsFinishing(true);
    try {
      const detalleParaHistorial = bottles.map(bot => {
        const t = totalsByBottle[bot.id] || { vendidos: 0, regalados: 0 }
        const stockApp = Number(bot.stockActual || 0)
        const fisico = parseFloat(conteoFisico[bot.id]) || 0
        
        return { 
          nombre: bot.nombre, 
          inicial: bot.stockInicialJornada || 0, 
          vendidos: t.vendidos, 
          regalados: t.regalados,
          esperado: stockApp, 
          fisico, 
          diferencia: fisico - stockApp 
        }
      })
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center p-20 gap-4">
      ...
      <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest italic">Sincronizando...</p>
    </div>
  )
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex justify-center sm:justify-start">
        <div className="flex bg-slate-900/80 p-1.5 rounded-[1.5rem] border-2 border-slate-800 shadow-xl">
          <button onClick={() => setActiveTab('auditoria')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase italic transition-all flex items-center gap-2 ${activeTab === 'auditoria' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}><BarChart3 size={14} /> Auditoría Actual</button>
          <button onClick={() => setActiveTab('historial')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase italic transition-all flex items-center gap-2 ${activeTab === 'historial' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}><History size={14} /> Historial</button>
        </div>
      </div>
        ...
        <>
          <div className="bg-slate-900/80 border-2 border-slate-800 p-6 rounded-[2.5rem] shadow-2xl">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-indigo-500/20 rounded-2xl flex items-center justify-center text-indigo-400 border border-indigo-500/30"><CalendarDays size={24} /></div>
                    <div className="text-left">
                        <h3 className="text-lg font-black text-white uppercase italic tracking-tighter leading-none">Fecha de Jornada</h3>
                        <p className="text-[9px] text-slate-500 font-bold uppercase mt-1 italic">Control de stock nocturno</p>
                    </div>
                </div>
                <div className="bg-slate-950 p-2 px-4 rounded-2xl border-2 border-slate-800 transition-all flex items-center gap-3">
          ...
          <div className="flex flex-col xl:flex-row justify-between items-center gap-6 bg-slate-900/50 p-8 rounded-[3rem] border-2 border-slate-800 shadow-2xl">
            <div className="flex items-center gap-5">
              <div className="w-14 h-14 bg-indigo-600 rounded-2xl flex items-center justify-center"><FileSpreadsheet size={28} className="text-white" /></div>
              <div className="text-left">
                <h2 className="text-2xl font-black uppercase italic leading-none text-white tracking-tighter">Conteo Físico</h2>
                <p className="text-[10px] text-slate-500 font-bold uppercase mt-2 flex items-center gap-2 italic"> <Info size={12} /> Compara stock de App vs Real </p>
              </div>
            </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-slate-900/80 text-[10px] text-slate-400 uppercase font-black tracking-widest border-b-2 border-slate-800">
                  ...
                  <th className="px-4 text-center text-purple-400">Regalos</th>
                  <th className="px-4 text-center">Esperado App</th>
                  <th className="px-4 text-center text-indigo-400 italic">Físico Real</th>
                ...
                {filteredBottles.map(bot => {
                  ...
                  return (
                    <tr key={bot.id} className="hover:bg-indigo-600/5 transition-colors group">
                      ...
                      <td className="px-4 text-center text-rose-500 font-black">-{t.vendidos.toFixed(1)}</td>
                      
                      <td className="px-4 text-center text-purple-400 font-black">
                        {t.regalados > 0 ? `-${t.regalados.toFixed(1)}` : '0.0'}
                      </td>
          ...
          <div className="flex justify-end mt-6">
            <button onClick={handleFinalizar} disabled={isFinishing} className={`bg-rose-600 hover:bg-rose-500 text-white px-10 py-5 rounded-[2rem] font-black uppercase italic transition-all flex items-center gap-3 shadow-xl active:scale-95 ${isFinishing ? 'opacity-50' : ''}`}>
              {isFinishing ? <Loader2 className="animate-spin" size={20} /> : <LogOut size={20} />} Finalizar Jornada
            </button>
          </div>
        </>
      ) : (
