          ...
          <div className="flex bg-slate-900/50 p-1.5 rounded-2xl border-2 border-slate-800 w-full md:w-auto overflow-x-auto no-scrollbar">
            {isAdmin && (
              <button 
                ...
                className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-[13px] font-black uppercase transition-all whitespace-nowrap ${activeTab === 'metrics' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
              >
                Métricas
              </button>
            )}
            {isAdmin && (
              <button 
                ...
                className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-[13px] font-black uppercase transition-all whitespace-nowrap ${activeTab === 'prices' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
              >
                Márgenes
              </button>
            )}
            {isAdmin && (
              <button 
                ...
                className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-[13px] font-black uppercase transition-all whitespace-nowrap ${activeTab === 'sales_detail' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
              >
                Detalle Ventas
              </button>
            )}
            {isAdmin && (
              <button 
                ...
                className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-[13px] font-black uppercase transition-all whitespace-nowrap ${activeTab === 'stock_control' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
              >
                Control Stock
              </button>
            )}
            {isAdmin && (
              <button 
                ...
                className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-[13px] font-black uppercase transition-all whitespace-nowrap ${activeTab === 'audit' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
              >
                Auditoría
              </button>
            )}
            <button 
              ...
              className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-xl text-[13px] font-black uppercase transition-all whitespace-nowrap ${activeTab === 'cash' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
            >
              Arqueo
            </button>
          </div>
...
<div className={`flex items-center justify-center gap-2 bg-slate-900/50 p-2 rounded-[2rem] border-2 border-slate-800 w-full sm:w-auto transition-all ${['cash', 'stock_control'].includes(activeTab) ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>          <Calendar className="w-4 h-4 text-indigo-500 ml-1 md:ml-2" />
          <input 
            ...
            className="bg-transparent font-black text-[13px] md:text-sm outline-none p-1 w-24 md:w-28 text-white appearance-none" 
          />
      ...
      {loading ? (
        <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-[13px] font-black uppercase tracking-widest text-slate-500">Sincronizando datos...</p>
        </div>
      ) : (
